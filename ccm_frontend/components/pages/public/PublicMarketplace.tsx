...
export function PublicMarketplace() {
  ...
  const handleRequireLogin = (action: string, creditId?: string) => {
    toast.info('Vui lòng đăng nhập để tiếp tục', {
      description: 'Bạn cần có tài khoản Người Mua để thực hiện hành động này',
      action: {
        label: 'Đăng nhập',
  ...
  return (
    <PublicLayout>
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl mb-4">🌿 Thị trường Tín chỉ Carbon</h1>
          <p className="text-lg text-muted-foreground">
            Khám phá và mua tín chỉ carbon từ các chủ xe điện đã được xác minh
          </p>
          <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm">
              💡 <strong>Lưu ý:</strong> Bạn cần <button onClick={() => navigateTo('/login')} className="text-blue-600 underline">đăng nhập</button> hoặc <button onClick={() => navigateTo('/register')} className="text-blue-600 underline">đăng ký tài khoản Người Mua</button> để có thể mua tín chỉ hoặc tham gia đấu giá.
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-3">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-muted-foreground">
                Hiển thị <strong>{filteredCredits.length}</strong> tín chỉ
              </p>
            </div>
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <Badge className={credit.type === 'direct' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        <Button 
                          variant="outline" 
                          className="flex-1"
                          onClick={() => handleRequireLogin('view', credit.id)}
                        >
                        ...
                        {credit.type === 'direct' ? (
                          <Button 
                            ...
                            onClick={() => handleRequireLogin('buy', credit.id)}
                          >
                          ...
                          <Button 
                            ...
                            onClick={() => handleRequireLogin('bid', credit.id)}
                          >
